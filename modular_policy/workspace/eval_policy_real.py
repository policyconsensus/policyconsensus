...
@click.command()
@click.option('--input', '-i', default='output/20250908/175602_train_dp_unet_rgb_marker-expert_N71/checkpoints/ep-1000_sr-0.999.ckpt', help='Path to checkpoint')
@click.option('--output', '-o', default='output/eval_marker_0825', help='Directory to save recording')
...
@click.option('--reset_robot', '-rr', default=False, type=bool)
@click.option('--debug', '-d', default=False, type=bool)
@click.option('--use_tactile', '-ut', default=True, help="Use tactile sensors.")
@click.option('--show_tactile_viz', default=False, type=bool, help="Show tactile visualization window.")
...
def eval_policy_real(input, output, robot_ip, match_dataset, match_episode,
    vis_cameras, init_joints,
    steps_per_inference, max_duration,
    frequency, command_latency,
    use_gripper, human_ctrl_mode, policy_ctrl_mode,
    delta_action,
    action_offset,
    reset_robot,
    debug,
    ...
    if hasattr(cfg, 'name') and 'tactile' in cfg.name:
        is_tactile_policy = True
    elif 'tactile' in str(input).lower():  # Check if checkpoint path contains 'tactile'
        is_tactile_policy = True
    
    if is_tactile_policy and hasattr(policy, 'modalities') and 'tactile' in policy.modalities:
        expected_tactile_keys = ['left_tactile', 'right_tactile']
        missing_tactile_keys = [key for key in expected_tactile_keys if key not in policy.obs_ports]
        if missing_tactile_keys:
            print(f"COMPATIBILITY FIX: Adding missing tactile observation ports: {missing_tactile_keys}")
            policy.obs_ports.extend(missing_tactile_keys)
            print(f"Updated observation ports: {policy.obs_ports}")
    ...
    with SharedMemoryManager() as shm_manager:
        ...
        with Spacemouse(shm_manager=shm_manager, use_gripper=use_gripper) as device, \
            ...
            with torch.no_grad():
                ...
                # print(f"Normalizer params keys: {list(policy.normalizer.params_dict.keys())}")
                
                # Filter observation ports to only those supported by the normalizer
                policy_expected_ports = policy.get_observation_ports()
                available_ports = list(obs_dict_np.keys())
                
                # Include ports that are both expected by policy and available in observation AND in normalizer
                supported_ports = [port for port in policy_expected_ports 
                                 if port in obs_dict_np and port in policy.normalizer.params_dict]
                
                print(f"Policy expects: {policy_expected_ports}")
                print(f"Available in obs: {available_ports}")
                print(f"In normalizer: {list(policy.normalizer.params_dict.keys())}")
                print(f"Using observation ports: {supported_ports}")
                ...
                # Use torch.clamp instead of np.clip to keep tensors on GPU (optional for tactile-only models)
                if 'camera_0_color' in obs_dict:
                    obs_dict['camera_0_color'] = torch.clamp(obs_dict['camera_0_color'] * 255, 0, 255).to(torch.uint8)
                if 'camera_1_color' in obs_dict:
