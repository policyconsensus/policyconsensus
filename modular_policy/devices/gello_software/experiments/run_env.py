...
def main(args):
    ...
    if args.bimanual:
        if args.agent == "gello":
            ...
            left = gello_config['left_gello_port']   
            right = gello_config['right_gello_port']
        ...
        reset_joints_left = np.deg2rad(gello_config['left_reset_joints'])
        reset_joints_right = np.deg2rad(gello_config['right_reset_joints'])
        reset_joints = np.concatenate([reset_joints_left, reset_joints_right])
        curr_joints = env.get_obs()["joint_positions"]
        max_delta = (np.abs(curr_joints - reset_joints)).max()
        steps = min(int(max_delta / 0.005), 500)
        ...
        if args.agent == "gello":
            ...
            if args.start_joints is None:
                robot_type = gello_config['single_arm_type']
                reset_joints = np.deg2rad(gello_config[f"{robot_type}_reset_joints"])
            else:
                reset_joints = args.start_joints
            agent = GelloAgent(port=gello_port, start_joints=args.start_joints)
            curr_joints = env.get_obs()["joint_positions"]
            curr_joints = np.array(curr_joints) if isinstance(curr_joints, list) else curr_joints
    ...
    start_pos = agent.act(env.get_obs()) # start_pos is the joint position of the gello agent
    obs = env.get_obs()
