...
class BimanualAgent(Agent):
    def __init__(self, agent_left: Agent, agent_right: Agent, enable_gripper: bool = True):
    ...
    def act(self, obs: Dict[str, Any]) -> np.ndarray:
        ...
        return np.concatenate(
            [self.agent_left.act(left_obs)[:int(self.dof/2)], self.agent_right.act(right_obs)[:int(self.dof/2)]]
        )
